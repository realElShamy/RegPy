...
This folder reverse engineers a widely used industry **case-study three-statement model** workbook and
...
| [`harness/case_study_inputs.json`](harness/case_study_inputs.json) | The reference case study reshaped as the prompt's parameterization payload (5 historical years + 5 years of forecast assumptions) |
...
- The harness run against the **original reference workbook**: **1,884 checks, 0 failures**
...
Everything in this folder is productized as a self-contained, harness-injectable
skill at [`../.claude/skills/three-statement-model/`](../.claude/skills/three-statement-model/):
a deterministic builder + validator pair driven by an orchestrating `SKILL.md`
pipeline that resolves the entity's jurisdiction (Egypt and GCC accounting/tax
packs: EAS, IFRS, Zakat, corporate tax, DMTT), pulls historicals from whatever MCP
data source is connected (accounting systems, drives, uploads) or manual input,
builds the workbook, gates it through the acceptance harness, and delivers it with
jurisdiction disclosures. Drop the skill folder into any agent harness's skills
directory to use it elsewhere.
...
python3 harness/validate_model.py MODEL.xlsx \
    ...
    [--reference harness/reference_values.json]   # regression vs the reference original
