# Reverse Engineering: CFI Case Study — Three Statement Model

A complete tear-down of `CFICaseStudyThreeStatementModel.xlsx` (creator: CFI — Corporate
Finance Institute). Every fact below was extracted programmatically from the workbook
...
*Cell comment on A102 (by CFI):* "We are using term debt/bond in this example so there's no
compounding interest. To learn more about revolver and compound interest, take our LBO
modeling course."
...
1. **Capex is displayed positive.** Row 72 "Investments in PP&E" = +15,000 and row 73
   ...
   `Net Δ Cash = CFO − CFI + CFF`. The chart block then re-negates it (`=-D73`) so the
...
1. **No circular reference.** Interest is charged on the *average* of opening and closing
   ...
   no revolver, no cash sweep), so the loop never closes. CFI's comment on A102 makes
...
4. **D&A quirk**: the historical ratio row computes D&A ÷ *closing* PP&E (`D32/D47`),
   ...
   simplification in the CFI case.
...
Web research (CFI's own guides plus industry modeling standards) corroborates every
structural choice observed in the file:

- **Build order.** CFI's published sequence — historicals → historical ratios → forecast
  ...
  (CFI, "3-Statement Model" guide).
- **Single sheet on purpose.** CFI "strongly recommends" the single-worksheet layout so
...
- **Interest convention.** Interest on the *average* debt balance is the more accurate
  convention but normally risks circularity; CFI's teaching materials discuss the
  ...
  is deferred to CFI's advanced courses).
